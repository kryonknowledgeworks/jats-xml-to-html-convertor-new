...
public class SubjectGroup implements Tag {
    private final Node node;

    public SubjectGroup(Node node) {
        this.node = node;
    }
    ...
    public String getElement() {
        ...
        for (int i = 0; i < node.getChildNodes().getLength(); i++) {
            ...
            } else if (nodeName.equalsIgnoreCase(Element.SUBJECT)) {
                stringBuilder.append(new Subject(childNode).getElement());
            } else if (nodeName.equalsIgnoreCase(Element.COMPOUND_SUBJECT)) {
